 ...
 * @tparam TP 
 * @param np 
 * @param paths : Es un paquete , que puede contener variables con diferentes tipos de datos, o funciones en un numero no definido. 
 * Esto indica que la funcion traversex, recibe un paquete de 0, 1 o n variables, segun se requiera
 * @return Node* 
 */
...
/*
    Se crea un arbol, basado en Nodos, cada Nodo puede tener varios Nodos a la vez.
    Revisar mejor esta funcion
    Enzo
...
void DemoFoldTraverse()
{
    ...
    /**
     * @brief ecfb
     * Se crea root que es un puntero hacia un nodo, y se inicializa como el valor de 5.
     */
    ...
    /**
     * @brief 
     * node es un puntero de tipo Node,  y se inicializa con la funcion traverse
     * 
     */
    Node* node = Node::traverse(root, Node::right, Node::left, Node::right);
    cout << node->getValue() << '\n';
    /*
    con el arbol creado, se asigna valores en deteminados nodos
    nodo ubicado en 3er nivel nodo derecho, 2do nodo izquierdo 3ero nodo derecho. nodo.derecho(nodoizquierdo(nodo.derecho))
    Se extra el valor del nodo ubicado.
    */
